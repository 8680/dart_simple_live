...
import 'package:simple_live_core/src/common/settings_service.dart';
...
class HuyaSite implements LiveSite {
  ...
  // CDN options mapping
  final Map<String, String> cdnMap = {
    "AL": "阿里",
    "TX": "腾讯",
    "HS": "火山",
  };
  
  // Cache of ping results
  Map<String, int>? _cdnPingResults;
  DateTime? _lastPingTime;
  String? _fastestCdn;
  
  // List of CDN domains to test
  final List<String> _cdnDomains = [
    "al.flv.huya.com",
    "tx.flv.huya.com",
    "hs.flv.huya.com",
  ];
  ...
  Future<List<String>> getPlayUrls(
    ...
    // Ping CDNs first to find the fastest one
    await _pingCdns();
    
    // Log the selected CDN
    if (_fastestCdn != null) {
      CoreLog.i("HUYA using fastest CDN: $_fastestCdn with latency: ${_cdnPingResults?[_fastestCdn] ?? 'unknown'}ms");
    }
    
    String? manualCdn = SettingsService().getHuyaCdn();
    if (manualCdn != null && manualCdn.isNotEmpty) {
      CoreLog.i("HUYA manual CDN override: $manualCdn");
    }
  ...
  Future<String> getPlayUrl(HuyaLineModel line, int bitRate) async {
    ...
    // Default to original CDN line
    String cdnLine = line.line;
    
    // Allow overriding the CDN by using a specific provider from settings
    String? selectedCdn = SettingsService().getHuyaCdn();
    if (selectedCdn != null && selectedCdn.isNotEmpty) {
      // Format: https://{cdn}.flv.huya.com/src/
      cdnLine = "https://$selectedCdn.flv.huya.com/src";
    } 
    // Use the fastest CDN if available and no manual override
    else if (_fastestCdn != null) {
      // _fastestCdn contains the full domain name, so we just need to add the protocol and path
      cdnLine = "https://$_fastestCdn/src";
    }
    
    var url =
        '$cdnLine/${resp.streamName}.flv?${resp.flvAntiCode}&codec=264';
    if (bitRate > 0) {
      url += "&ratio=$bitRate";
    }
    return url;
  ...
  /// Ping all CDN domains and find the one with lowest latency
  Future<void> _pingCdns() async {
    // Skip if we've already pinged recently (within last 5 minutes)
    if (_lastPingTime != null && 
        DateTime.now().difference(_lastPingTime!).inMinutes < 5 &&
        _fastestCdn != null) {
      return;
    }
    ...
    // Run ping tests in parallel for each CDN
    List<Future<void>> pingTasks = [];
    
    for (String domain in _cdnDomains) {
      pingTasks.add(_pingCdn(domain));
    }
    
    // Wait for all ping tests to complete
    await Future.wait(pingTasks);
    
    // Find the CDN with lowest latency
    if (_cdnPingResults?.isNotEmpty == true) {
      _cdnPingResults!.removeWhere((key, value) => value == -1); // Remove failed pings
      
      if (_cdnPingResults!.isNotEmpty) {
        var sortedEntries = _cdnPingResults!.entries.toList()
          ..sort((a, b) => a.value.compareTo(b.value));
        
        _fastestCdn = sortedEntries.first.key;
        _lastPingTime = DateTime.now();
      }
    }
  }
  
  /// Ping a single CDN domain and measure latency
  Future<void> _pingCdn(String domain) async {
    try {
      Stopwatch stopwatch = Stopwatch()..start();
      
      // Use a simple head request to test connectivity and measure latency
      // The actual endpoint doesn't matter as long as it gives us a network response
      final testEndpoint = "https://$domain/favicon.ico";
      
      await HttpClient.instance.getText(
        testEndpoint,
        timeout: const Duration(seconds: 3),
      ).catchError((e) {
        // We still count this as a successful ping if we got any response
        // Even a 404 is fine since we're just measuring network latency
        return "";
      });
      ...
      _cdnPingResults![domain] = latency;
      
      // Log the result
      CoreLog.d("HUYA CDN ping: $domain - ${latency}ms");
    } catch (e) {
      // If ping completely fails (timeout/network error), set to -1
      _cdnPingResults![domain] = -1;
      CoreLog.w("HUYA CDN ping failed: $domain - $e");
    }
  }
